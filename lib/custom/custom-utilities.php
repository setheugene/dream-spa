...
function get_full_string( $string, $start, $end ) {
  $string = ' ' . $string;
  $ini = strpos($string, $start);
  if ($ini == 0) return '';
  $ini += strlen($start);
  $len = strpos($string, $end, $ini) - $ini;
  $middle = substr($string, $ini, $len);
  return $start . $middle . $end;
}

add_filter( 'gform_field_groups_form_editor', function( array $field_groups ) {
  $blacklisted_fields = [
    'standard_fields' => [],
    'advanced_fields' => ['name', 'date', 'time', 'multiselect'],
    'post_fields' => [],
    'pricing_fields' => []
  ];

  foreach( $field_groups as $group => &$field_group ) {
    if ( !empty($blacklisted_fields[$group]) ) {
      $field_group['fields'] = array_values( array_filter( $field_group['fields'], function( $field ) use ( $blacklisted_fields, $group ) {
        return !in_array( $field['data-type'], $blacklisted_fields[$group] );
      } ) );
    }
  }
